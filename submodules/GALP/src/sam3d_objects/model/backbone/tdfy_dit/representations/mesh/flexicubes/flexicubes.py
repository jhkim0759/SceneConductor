...
try:
    ...
    def check_tensor(tensor, shape_spec, throw=True):
        """Lightweight fallback that checks rank and fixed dimensions."""
        if not torch.is_tensor(tensor):
            if throw:
                raise TypeError("Expected a torch.Tensor")
            return False
        if len(shape_spec) != tensor.dim():
            if throw:
                raise AssertionError(f"Expected rank {len(shape_spec)}, got {tensor.dim()}")
            return False
        for idx, (size, spec) in enumerate(zip(tensor.shape, shape_spec)):
            if spec is not None and size != spec:
                if throw:
                    raise AssertionError(f"Dimension {idx} expected {spec}, got {size}")
                return False
        return True
