...
class Latent_out(nn.Module):
    def __init__(
        self, in_channels, model_channels: int, pos_embedder: Callable[[], torch.Tensor]
    ):
        super().__init__()
        # Some checkpoints only provide the output projection, but the current
        # flow code expects an input projection too. Default to a learnable
        # linear layer here so downstream code can build inputs even when the
        # checkpoint did not save it.
