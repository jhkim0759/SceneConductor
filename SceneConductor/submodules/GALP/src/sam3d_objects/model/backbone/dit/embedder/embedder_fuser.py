...
class EmbedderFuser(torch.nn.Module):
    ...
    def _make_placeholder(self, embedder: nn.Module, batch_meta):
        """
        Build a dummy input so missing modalities still yield tokens.
        Keeps condition shapes consistent when a modality is absent.
        """
        batch_size, device, dtype = batch_meta
        if hasattr(embedder, "input_size"):
            size = embedder.input_size
        else:
            raise KeyError(
                "Cannot infer placeholder shape for embedder without `input_size` attribute"
            )

        # Dino and PointPatchEmbed both expect (B, 3, H, W)
        return torch.zeros(batch_size, 3, size, size, device=device, dtype=dtype)

    def _get_batch_meta(self, kwargs):
        for v in kwargs.values():
            if torch.is_tensor(v):
                return v.shape[0], v.device, v.dtype
        # fallback: single sample on module device
        return 1, self.idx_emb.device, torch.float32

    def forward(self, *args, return_dict: bool = False, **kwargs):
        """
        Args:
            return_dict: if True, return ``Dict[kwarg_name, token_tensor]`` instead of
                         the concatenated tensor.  Used by V2 training to separate
                         image tokens from pointmap tokens before cross-attention.
        """
        tokens = []
        kwarg_names = []
        batch_meta = self._get_batch_meta(kwargs)
        ...
        for i, (condition_embedder, kwargs_info) in enumerate(self.embedder_list):
            ...
            for kwarg_name, pos_group in kwargs_info:
                if kwarg_name not in kwargs:
                    # logger.warning(
                    #     f"{kwarg_name} not in kwargs to condition embedder! Using zeros placeholder."
                    # )
                    continue
                    # input_cond = self._make_placeholder(condition_embedder, batch_meta)
                else:
        ...
        # V2 split path: return per-key token tensors without concatenation
        if return_dict:
            return dict(zip(kwarg_names, tokens))
