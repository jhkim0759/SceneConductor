#!/usr/bin/env bash
# Re-render ONLY clips 4 (zoom) & 5 (turntable) for a scene with the current
# build_tour.py, then re-concat 1+2+3+4+5 -> tour_full.mp4. Reuses the existing
# clips 1,2,3 and the prep (g_init/g_final) from a prior make_tour run.
#   redo_tour45.sh <scene_dir> [samples]
set -u
...
if [ -z "$GINIT" ]; then echo "REDO45_SKIP $SCENE (no group json)"; exit 2; fi
...
echo "REDO45_DONE $SCENE -> $SV/tour_full.mp4"
