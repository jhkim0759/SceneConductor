...
def _move_closer_chamfer(anchor_pts, moving_pts):
    _, diff = _min_pair_diff(anchor_pts, moving_pts)
    return -diff
...
def _apply_action(op):
    ...
    if action == "flip_yaw_180":
        ...
        o.rotation_mode = "XYZ"
        before = list(o.rotation_euler)
        # Add π to current Z rotation, then normalize to (-π, π].
        new_rz = math.atan2(
    ...
    if action == "attach_to_wall":
        ...
        if not preserve_rotation:
            ...
            moving.rotation_euler = (before_rot[0], before_rot[1], tangent_angle)
            bpy.context.view_layer.update()
        ...
        # positive direction = inward (toward room interior).
        diff_xy = Vm[:, :2] - np.array([v_from[0], v_from[1]])
        d_signed = diff_xy[:, 0] * nx + diff_xy[:, 1] * ny
        d_min = float(d_signed.min())

        # STEP 4 — translate so the most-outward vertex sits at
        # (wall_thickness/2 + clearance) inward of the wall centerline.
        # Along-wall (t_along) position: use forced_t when given, else preserve current.
        target_perp = clearance  # polygon_vertices are at inner wall surface (Solidify offset=-1.0)
