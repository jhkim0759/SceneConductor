#!/usr/bin/env python3
"""Crop per-object thumbnails from image.png using inputs/masks/<id>.png.

Outputs `inputs/thumbnails/obj_<id>_<class>.png`. Skips existing files unless
--force is passed. The thumbnail filename embeds the class so the downstream
`stage3-relation-graph` agent and other Stage-3 consumers can identify objects
by glancing at file paths.
"""
import argparse
import json
import re
...
def main() -> int:
    ...
    if not args.force:
        try:
            thumb_dir = scene / "inputs" / "thumbnails"
            existing = list(thumb_dir.glob("obj_*.png")) if thumb_dir.is_dir() else []
            if existing:
                n = len(existing)
                print(
                    f"[make_thumbnails] reusing cached {thumb_dir} from Stage 1 "
                    f"({n} thumbnails); skipping regeneration"
                )
                return 0
        except Exception:
    ...
    class_path = scene / "inputs" / "object_class.json"
    out_dir = scene / "inputs" / "thumbnails"
    ...
    print(f"done: made={n_made} skipped={n_skipped} empty={n_empty}")
    print(f"thumbnails dir: {out_dir}")
    return 0
