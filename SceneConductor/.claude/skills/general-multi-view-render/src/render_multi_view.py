"""render_multi_view.py — stage-agnostic multi-view Cycles renderer.

Renders 5 look-dev views of any populated `.blend` (Stage 1 / Stage 2 /
Stage 3 alike). Most views use the scene's existing (calibrated) lighting;
the BEV view temporarily swaps to an albedo-style render (Standard view
transform + flat-white world at strength=π) so floor / stage-top base
colors are legible from above.
...
def _parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        ...
        description="Render 4 Cycles multi-view PNGs from the env-enhanced .blend.",
    )
    ap.add_argument(
        "--scene-dir",
        required=True,
...
def render_bev(
    ...
    """View 2 — top-down orthographic floor-plan view, rendered as ~albedo.

    Designed as a diagnostic top-down map, not a photoreal interior shot.  We
    deliberately bypass the calibrated AgX pipeline so surface base colors are
    legible:

      - view_transform = Standard, look = None (no AgX rolloff / contrast).
      - exposure = 0.
      - cycles.sample_clamp_indirect = 0 + diffuse/glossy bounces clamped to 1
        so colors come overwhelmingly from direct + 1-bounce world emission,
        not multi-bounce GI tinting.
      - World background = flat white at strength ~ pi (≈ 3.14) — a white world
        emitting at strength=pi reproduces an albedo render: a perfectly diffuse
        floor with base_color C renders as exactly C in linear RGB.
      - Ceiling hidden so the ortho camera sees the floor.
      - All existing scene lights disabled (calibrated for the perspective
        vantage, would produce a hotspot from above).
      - Camera: temp ortho, (cx, cy, hi.z + 5.0) looking straight down.
    ...
    print(
        ...
        f"ortho_scale={ortho_scale:.2f}  lighting=bev_albedo  "
    ...
    # --- Snapshot: color management + cycles bounce settings ---
    orig_view_transform = scene.view_settings.view_transform
    orig_look = scene.view_settings.look
    orig_exposure = scene.view_settings.exposure
    orig_diffuse_bounces = scene.cycles.diffuse_bounces
    orig_glossy_bounces = scene.cycles.glossy_bounces
    orig_transmission_bounces = scene.cycles.transmission_bounces
    orig_sample_clamp_indirect = scene.cycles.sample_clamp_indirect

    # Albedo-friendly color management: bypass AgX so RGB shows the actual
    # base color, not tonemapped highlights.
    scene.view_settings.view_transform = "Standard"
    scene.view_settings.look = "None"
    scene.view_settings.exposure = 0.0
    # Keep colors close to albedo: 1 diffuse bounce = direct world-emission
    # shading only (a pure 0-bounce render renders surfaces black under world-
    # only lighting). Glossy/transmission stay low to avoid tint creep.
    scene.cycles.diffuse_bounces = 1
    scene.cycles.glossy_bounces = 1
    scene.cycles.transmission_bounces = 1
    scene.cycles.sample_clamp_indirect = 1.0

    # --- Replace world with flat white emission at strength=pi (albedo). ---
    # Rationale: a perfectly Lambertian surface with base color C, illuminated
    # by a uniform world emitting at strength=pi, renders to exactly C in linear
    # RGB. Combined with Standard view transform, the BEV image is then a true
    # albedo map of the floor and stage tops.
    world.use_nodes = True
    nt = world.node_tree
    nt.nodes.clear()
    bg_node = nt.nodes.new("ShaderNodeBackground")
    bg_node.inputs["Color"].default_value = (1.0, 1.0, 1.0, 1.0)
    bg_node.inputs["Strength"].default_value = math.pi
    ...
    try:
        ...
        # Restore color management + cycles bounce settings
        scene.view_settings.view_transform = orig_view_transform
        scene.view_settings.look = orig_look
        scene.view_settings.exposure = orig_exposure
        scene.cycles.diffuse_bounces = orig_diffuse_bounces
        scene.cycles.glossy_bounces = orig_glossy_bounces
        scene.cycles.transmission_bounces = orig_transmission_bounces
        scene.cycles.sample_clamp_indirect = orig_sample_clamp_indirect
...
def main() -> None:
    ...
    # Temporarily lower samples
    orig_samples: Optional[int] = None
    if hasattr(scene.cycles, "samples"):
        orig_samples = scene.cycles.samples
        if args.samples != orig_samples:
            scene.cycles.samples = args.samples
            print(f"[render_multi_view] samples: {orig_samples} → {args.samples}")
    ...
    # Output paths — all views go into <scene_dir>/render/
    ...
    try:
        ...
        if orig_samples is not None and scene.cycles.samples != orig_samples:
            scene.cycles.samples = orig_samples
            print(f"[render_multi_view] samples restored to {orig_samples}")
    ...
    for label, p in (
        ...
        ("bev                 (albedo render: Standard view, flat-white world)       ", out_bev),
